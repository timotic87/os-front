...
Ovaj fajl pruža uputstva za WARP (warp.dev) pri radu sa kodom u ovom repozitorijumu.

## Pregled Projekta
OSAngular je Angular 17 enterprise aplikacija za upravljanje klijentima, poslovima i poslovnim tokovima. Aplikacija koristi Firebase za skladištenje fajlova, Socket.IO za real-time notifikacije, i Material Angular + Tailwind CSS za UI komponente.
...
ng serve
```
Development server radi na `http://localhost:4200/` sa hot reload-om.
...
```
Build artifakti se čuvaju u `./build/` direktorijumu (konfigurisano u angular.json).

### Testiranje
```bash
# Pokretanje svih unit testova preko Karma
npm test
# ili
ng test
```
